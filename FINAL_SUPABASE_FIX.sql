-- COMPLETE & FINAL SUPABASE SETUP SCRIPT
...
CREATE TABLE IF NOT EXISTS public.patients (
    id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
    patient_id text NOT NULL DEFAULT 'PAT-' || substr(gen_random_uuid()::text, 1, 8),
    name text NOT NULL,
    email text,
    phone text,
    age integer,
...
-- 5. Create Policies (Cleanup and Recreate)
DO $$ BEGIN
    ...
    -- ... (Dropping others implicitly by overwriting or they are non-existent on fresh db)
EXCEPTION
...
CREATE POLICY "everyone view transactions" ON public.transactions FOR
SELECT USING (true);
...
-- 7. CREATE ADMIN USER (admin@123.com)
-- NOTE: We append .com to ensure valid email format for Supabase
DO $$
DECLARE
  new_admin_id uuid := gen_random_uuid();
  admin_email text := 'admin@123.com'; 
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = admin_email) THEN
    ...
    INSERT INTO auth.users (
      ...
      new_admin_id, '00000000-0000-0000-0000-000000000000', 
      'authenticated', 'authenticated', admin_email, 
      crypt(admin_password, gen_salt('bf')), now(), 
      '{"provider":"email","providers":["email"]}', '{"full_name":"System Admin"}', 
      now(), now(), '', '', false
    );
